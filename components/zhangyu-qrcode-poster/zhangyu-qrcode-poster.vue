...
<script>
	export default{
		...
		methods:{
			//显示
			showCanvas(qrcode,qrcodebase64){
				this.isShow = true
				this.qrcode = qrcode
			...
			async __init(){
				...
				var urlQRBase64= 'data:image/jpg;base64,'+this.qrcodebase64;	//urlQRBase64  base64数据  （如果没有前缀，自行加上前缀）
				var urlQR = await  this.base64ToSave(urlQRBase64);
				...
				//获取标题图片
				let headerImg = this.headerImg//await this.getImageInfo(this.headerImg)
				let hW = uni.upx2px(500);
				let hH = uni.upx2px(168);
				//绘制标题图
				this.drawRoundImg(this.ctx,headerImg,((this.canvasW-hW) / 2),((this.canvasW-hW) / 2),hW,hH,uni.upx2px(16))
				...
				this.ctx.drawImage(urlQR,uni.upx2px(33),(((this.canvasW-hW) / 2) + hH + uni.upx2px(22)), uni.upx2px(500), uni.upx2px(500))
				this.ctx.setFontSize(12);
				this.ctx.setFillStyle('#000000');
				this.ctx.fillText('保存图片,邀请约车',(((this.canvasW-hW) / 2)+ hH - uni.upx2px(22) ),(((this.canvasW-hW) / 2) + hH + uni.upx2px(530)))
				//延迟渲染
				setTimeout(()=>{
					this.ctx.draw(true,()=>{
						uni.hideLoading()
